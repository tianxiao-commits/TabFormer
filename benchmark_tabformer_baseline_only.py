"""
TabFormer Baseline-Only Benchmark Sweep

Tests baseline (no torch.compile) version only.
Uses BF16 precision without torch.compile optimization.
...
def find_max_batch_size_for_sla(
    ...
    """
    Find maximum batch size that meets SLA (baseline, no torch.compile).

    Strategy:
    - Batch sizes: 1, 2, 3, 4, 6, 8, 10, 12, 14, 16, 20, 24, 28, 32, 40, 48, 56, 64, 80, 96...
    - Create fresh model for each batch_size
    ...
    while batch_size <= max_batch_size:
        try:
            # Create fresh model for each batch_size (baseline, no compile)
            ...
            metrics = benchmark_model(
                ...
                batch_size,
                seq_len,
                warmup_iters=5,  # Baseline doesn't need CUDA graph warmup
                bench_iters=50,
                device=device,
                use_bf16=use_bf16,
            )
...
def run_sweep(
    ...
    """Run SLA-based sweep for a model config (baseline only)."""
    print(f"\n{'='*80}")
    print(f"Running baseline sweep for {config_name} TabFormer")
    ...
    for seq_len in seq_lengths:
        ...
        try:
            ...
            print(f"  Baseline (no torch.compile, BF16):")
