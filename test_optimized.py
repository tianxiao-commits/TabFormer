"""
Test optimized (torch.compile + CUDA graphs) for nsys profiling.
...
def main():
    ...
    print("="*80)
    print("OPTIMIZED (torch.compile + CUDA graphs)")
    print("="*80)
    ...
    print(f"Warming up (compiling + capturing CUDA graphs)...")
    with torch.no_grad():
        for i in range(warmup_iters):
            _ = model(input_ids)
            if i == 0:
                print(f"  First iteration complete (compilation triggered)")
